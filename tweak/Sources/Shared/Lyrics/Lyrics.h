...
// Spotify's Authorization header, nil until one has gone out. Used for Spotify requests and, only
// when the user enables Spicy Lyrics, sent to that lyrics API for authentication.
...
// The lyric cache key: a Spotify base62 id, or a local URI while local-file lyrics are enabled.
